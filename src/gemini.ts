...
export interface GeminiTryonInput {
  /** Imagen del usuario en base64 (sin prefijo data URL). */
  userImageBase64: string;
  /** MIME type de la imagen del usuario (ej. image/jpeg). */
  userImageMimeType: string;
  /** URL de la imagen del producto (se descarga y se envía a Gemini). */
  productImageUrl: string;
  /** Referencia del producto (ej. "Camiseta básica algodón"); firstWord = primera palabra. */
  referencia: string;
}
 ...
 */
function getFirstWord(referencia: string): string {
 ...
 */
function buildTryonPrompt(firstWord: string, productName: string): string {
  return `
 ...
 */
async function fetchProductImageAsBase64(productImageUrl: string): Promise<{ data: string; mimeType: string }> {
...
export async function runGeminiTryon(input: GeminiTryonInput): Promise<string> {
  ...
  logInfo("gemini tryon fetch product image", { urlLength: input.productImageUrl.length });
  ...
  logInfo("gemini tryon generateContent", {
    model: config.geminiTryonModel,
    firstWord,
  ...
  if (!candidates || candidates.length === 0) {
    ...
    logError("gemini tryon no candidates", { blockReason });
  ...
  if (!lastImageBase64) {
    logError("gemini tryon no image in response", { partsCount: parts.length });
    throw new Error("Gemini did not return an image");
  }

  logInfo("gemini tryon success", { imageBase64Length: lastImageBase64.length });
